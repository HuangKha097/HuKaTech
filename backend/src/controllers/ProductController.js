...
const addNewProduct = async (req, res) => {
  try {
    const { name, image, type, newPrice, countInStock, description, category } =
      req.body;
    if (
      !name ||
      !image ||
      ...
      return res.status(200).json({
        status: "Error",
        message:
          "The category must be one of these values('Hot Deals', 'Discounts', 'New Arrivals', 'Black Friday Deals')",
      });
    }
